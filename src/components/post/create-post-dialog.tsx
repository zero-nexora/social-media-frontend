...
import { Globe, Users, Lock, Image as ImageIcon, AlertCircle } from "lucide-react";
...
export const CreatePostDialog = ({ open, onClose, onSuccess, initialMediaOpen }: Props) => {
  const { user } = useAuth();
    ...
    onSuccess: (post) => {
      ...
      queryClient.setQueriesData<any>({ queryKey: ["feed"] }, (old: any) => {
        ...
        return {
          ...old,
          pages: [
            { data: [post, ...(old.pages[0]?.data ?? [])], nextCursor: old.pages[0]?.nextCursor, hasMore: old.pages[0]?.hasMore },
            ...old.pages.slice(1),
          ],
        };
      });
  ...
  const canSubmit = (plainText.trim().length > 0 || upload.hasFiles) && !isWorking && !upload.hasError;
    ...
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="sm:max-w-145 max-h-[90vh] flex flex-col gap-0 p-0 overflow-hidden">
        <DialogHeader className="px-5 pt-5 pb-3 border-b">
          <DialogTitle className="text-center text-base font-semibold">Tạo bài viết</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <PostEditor
            placeholder={user ? `Bạn đang nghĩ gì, ${user.username}?` : "Bạn đang nghĩ gì?"}
            onChange={(h, t) => { setHtml(h); setPlainText(t); }}
            onReset={(fn) => { editorResetRef.current = fn; }}
            autoFocus={open}
          />
          ...
          {upload.hasError && (
            <div className="flex items-start gap-2 text-sm text-destructive bg-destructive/8 rounded-xl px-3 py-2.5 border border-destructive/15">
              ...
              <span>Một số file upload thất bại. Xoá và thử lại hoặc chọn file khác.</span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t bg-card">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" onClick={handleClose} disabled={isWorking}>
              Huỷ
            </Button>
            <Button onClick={() => createMutation.mutate()} disabled={!canSubmit} className="min-w-18">
              {buttonLabel()}
            </Button>
          </div>
        </div>
